...
- Community Post/Post Reply/Post Reaction/Post Tag: a feed where people share a good deed or ask for help, answer each other, and offer support. **Safety by design** (see `docs/community-safety.md`): the primary shared object is an app-generated **Kindness Card** built from a catalog Deed Type (its illustration + title + category), not a user upload — there is no image-upload path at all. Free text is limited to a short optional note and short replies that pass a pre-publish filter (no links, length-capped, rate-limited). This gives a RedNote-style visual feel without the RedNote content-moderation risk surface. Shape follows conventions common to open-source forums (Discourse, Lemmy): a post owns its replies, carries a kind (`share` | `ask`) and an optional project tag, and holds a moderation status from creation so reporting withdraws it immediately. Support is deliberately one-press-per-person and reversible — the product avoids leaderboards and pile-on scoring. Implemented in `prototype/app/community.js`, specified test-first in `tests/browser/foobow.community.spec.mjs`.
